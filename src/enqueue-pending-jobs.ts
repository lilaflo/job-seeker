...
async function enqueuePendingJobs() {
  try {
    ...
    const pendingJobs = allJobs.filter(job => job.processing_status === 'pending');
    ...
    for (const job of pendingJobs) {
      try {
        ...
        await enqueueJobProcessing(job.id, job.title, job.link, job.email_id || undefined);
        enqueued++;
      } catch (error) {
    ...
    console.debug(`✓ Successfully enqueued ${enqueued}/${pendingJobs.length} jobs`);
